#!/usr/bin/env bash

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INIT_DB_SCRIPT="${SCRIPT_DIR}/../scripts/init-db.sh"
DATA_DIR="${HOME}/.claude/interstat"
DB_PATH="${DATA_DIR}/metrics.db"
FAILED_INSERTS_PATH="${DATA_DIR}/failed_inserts.jsonl"

INPUT=$(cat)

session_id="$(printf '%s' "$INPUT" | jq -r '(.session_id // "")' 2>/dev/null || printf '')"
agent_name="$(printf '%s' "$INPUT" | jq -r '(.tool_input.subagent_type // "unknown")' 2>/dev/null || printf 'unknown')"
tool_output="$(printf '%s' "$INPUT" | jq -r '(.tool_output // "")' 2>/dev/null || printf '')"
result_length="$(printf '%s' "$tool_output" | wc -c | tr -d '[:space:]')"
invocation_id="$(cat /proc/sys/kernel/random/uuid 2>/dev/null || printf '')"
timestamp="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"

if [ -z "${agent_name}" ] || [ "${agent_name}" = "null" ]; then
  agent_name="unknown"
fi

if [ -z "${result_length}" ]; then
  result_length=0
fi

mkdir -p "$DATA_DIR" >/dev/null 2>&1 || true
bash "$INIT_DB_SCRIPT" >/dev/null 2>&1 || true

escaped_timestamp="$(printf "%s" "$timestamp" | sed "s/'/''/g")"
escaped_session_id="$(printf "%s" "$session_id" | sed "s/'/''/g")"
escaped_agent_name="$(printf "%s" "$agent_name" | sed "s/'/''/g")"
escaped_invocation_id="$(printf "%s" "$invocation_id" | sed "s/'/''/g")"

insert_status=0
sqlite3 "$DB_PATH" <<SQL >/dev/null 2>&1 || insert_status=$?
PRAGMA busy_timeout=5000;
INSERT INTO agent_runs (
  timestamp,
  session_id,
  agent_name,
  invocation_id,
  result_length
) VALUES (
  '${escaped_timestamp}',
  '${escaped_session_id}',
  '${escaped_agent_name}',
  '${escaped_invocation_id}',
  ${result_length}
);
SQL

if [ "$insert_status" -ne 0 ]; then
  parsed_input="$(printf '%s' "$INPUT" | jq -c '.' 2>/dev/null || printf 'null')"
  if ! fallback_payload="$(jq -cn \
    --arg timestamp "$timestamp" \
    --arg session_id "$session_id" \
    --arg agent_name "$agent_name" \
    --arg invocation_id "$invocation_id" \
    --arg raw_input "$INPUT" \
    --argjson result_length "$result_length" \
    --argjson input_json "$parsed_input" \
    '{
      timestamp: $timestamp,
      session_id: $session_id,
      agent_name: $agent_name,
      invocation_id: $invocation_id,
      result_length: $result_length,
      input: $input_json,
      raw_input: $raw_input
    }' 2>/dev/null)"; then
    escaped_raw_input="$(printf '%s' "$INPUT" | sed 's/\\/\\\\/g; s/"/\\"/g')"
    fallback_payload="{\"raw_input\":\"${escaped_raw_input}\"}"
  fi
  printf '%s\n' "$fallback_payload" >> "$FAILED_INSERTS_PATH" 2>/dev/null || true
fi

exit 0
